...
const RPC_JOIN_OR_CREATE_NAME = "physics_match"
const RPC_KILL_MATCH_NAME = "physics_kill"
...
func InitModule(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, initializer runtime.Initializer) error {
	...
	if err := initializer.RegisterRpc(RPC_JOIN_OR_CREATE_NAME, PhysicsMatchRPC); err != nil {
		logger.Error("Unable to register: %v", err)
		return err
	}

	if err := initializer.RegisterRpc(RPC_KILL_MATCH_NAME, KillPhysicsMatchesRPC); err != nil {
		logger.Error("Unable to register: %v", err)
		return err
	}
...
func PhysicsMatchRPC(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, payload string) (string, error) {
	var min_size *int
	...
	*max_size = 2
	...
	if matches, err := nk.MatchList(ctx, limit, true, label, min_size, max_size, "+label.open:1 +label.physics:1"); err != nil {
...
func KillPhysicsMatchesRPC(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, payload string) (string, error) {
	var minSize *int
	var maxSize *int
	var label string
	if matches, err := nk.MatchList(ctx, 20, true, label, minSize, maxSize, "+label.physics:1"); err != nil {
		logger.Error("[MatchList]: %s", err)
	} else {
		if len(matches) > 0 {
			for _, match := range matches {
